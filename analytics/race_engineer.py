...
def _analyze_comfort_and_acceleration(r: QuarterCarResult) -> List[Recommendation]:
    ...
    if r.rms_body_accel > 1.5:
        recs.append(Recommendation(
            category="RIDE COMFORT",
            diagnosis="Severe Ride Harshness",
            finding=f"RMS Body Acceleration ({r.rms_body_accel:.2f} m/s²) exceeds ISO limits for comfort.",
            action="Soften spring rate (ks) and reduce high-speed compression damping.",
            severity=Severity.CRITICAL,
            confidence=0.95
        ))
    elif r.rms_body_accel > 0.8:
        recs.append(Recommendation(
            category="RIDE COMFORT",
            diagnosis="Elevated Ride Harshness",
            finding=f"RMS Body Acceleration ({r.rms_body_accel:.2f} m/s²) is slightly uncomfortable.",
...
def _analyze_compound_issues(r: QuarterCarResult) -> List[Recommendation]:
    ...
    if r.rms_body_accel > 1.5 and peak_travel_mm > 75.0:
        recs.append(Recommendation(
            category="SYSTEM CONFLICT",
            diagnosis="Bottoming-Induced Harshness",
            finding=f"Severe harshness ({r.rms_body_accel:.2f} m/s²) combined with massive travel ({peak_travel_mm:.1f} mm). The harshness is caused by the suspension violently bottoming out, NOT by the springs being too stiff.",
